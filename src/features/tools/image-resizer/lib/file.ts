...
export function getAspectRatioHeight(width: number, originalWidth: number, originalHeight: number): number {
  if (originalWidth <= 0) return originalHeight
  return Math.max(1, Math.round((width / originalWidth) * originalHeight))
}

export function getAspectRatioWidth(height: number, originalWidth: number, originalHeight: number): number {
