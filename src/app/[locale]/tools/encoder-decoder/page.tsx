...
import { EncoderDecoderTool } from '@/features/tools/encoder-decoder'
import {
  getCanonicalUrl,
  getDefaultLocaleUrl,
...
const pathname = '/tools/encoder-decoder'
...
export default async function EncoderDecoderPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  const jsonLd = [
    {
      ...
      featureList: [t('seo.modes'), t('seo.privacy'), ...useCases],
      inLanguage: locale,
      isPartOf: {
        '@type': 'WebSite',
      ...
      <Container>
        <section className="pb-14">
          <div className="grid gap-8 md:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="space-y-6">
              <div>
                <h2 className="text-base font-semibold text-text-main">{t('seo.modesTitle')}</h2>
                <p className="mt-2 text-sm text-mist">{t('seo.modes')}</p>
              </div>
              <div>
