...
test('잘못된 segment는 400을 반환', async () => {
  const response = await GET(new Request('https://oddkit.tools/placeholder/abcxdef.png'), makeContext('abcxdef.png'))
...
test('허용되지 않은 포맷은 400을 반환', async () => {
  const response = await GET(new Request('https://oddkit.tools/placeholder/600x400.bmp'), makeContext('600x400.bmp'))
  expect(response.status).toBe(400)
  const body = await response.json()
...
test('유효한 PNG 요청은 200과 올바른 content-type을 반환', async () => {
  const response = await GET(new Request('https://oddkit.tools/placeholder/600x400.png'), makeContext('600x400.png'))
...
test('유효한 JPG 요청은 image/jpeg content-type을 반환', async () => {
  const response = await GET(new Request('https://oddkit.tools/placeholder/300x200.jpg'), makeContext('300x200.jpg'))
  expect(response.status).toBe(200)
  expect(response.headers.get('Content-Type')).toBe('image/jpeg')
})

test('유효한 WebP 요청은 image/webp content-type을 반환', async () => {
  const response = await GET(new Request('https://oddkit.tools/placeholder/300x200.webp'), makeContext('300x200.webp'))
  expect(response.status).toBe(200)
